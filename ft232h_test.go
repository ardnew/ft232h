...
func TestMain(m *testing.M) {
	...
	flag.BoolVar(&optNoDevice, "nodevice", defNoDevice, "do not expect to open an FT232H")
...
func TestNewFT232HWithIndex(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "selected", index: optIndex, open: !optNoDevice},
			...
			func(s *testing.T) {
				...
				if test.open {
					...
					if nil == err { // fail
						ft.Close()
						if optNoDevice {
							s.Fatalf("opened device at index=%d with -nodevice set", test.index)
						} else {
...
func TestNewFT232HWithVIDPID(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "selected", vid: optVID, pid: optPID, open: !optNoDevice},
			...
			func(s *testing.T) {
				...
				if test.open {
					...
					if nil == err { // fail
						ft.Close()
						if optNoDevice {
							s.Fatalf("opened device at with VID=0x%X, PID=0x%X and -nodevice set",
								test.vid, test.pid)
						} else {
...
func TestNewFT232HWithSerial(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "selected", serial: optSerial, open: !optNoDevice},
		{name: "empty", serial: "", open: !optNoDevice},
			...
			func(s *testing.T) {
				...
				if test.open {
					...
					if nil == err { // fail
						ft.Close()
						if optNoDevice {
							s.Fatalf("opened device with serial=\"%s\" and -nodevice set",
								test.serial)
						} else {
...
func TestNewFT232HWithDesc(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "selected", desc: optDesc, open: !optNoDevice},
		{name: "empty", desc: "", open: !optNoDevice},
			...
			func(s *testing.T) {
				...
				if test.open {
					...
					if nil == err { // fail
						ft.Close()
						if optNoDevice {
							s.Fatalf("opened device with description=\"%s\" and -nodevice set",
								test.desc)
						} else {
...
func TestParseUint32(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "", str: "0b10101010101010101010101010101010", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "025252525252", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "0o25252525252", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "2863311530", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "0xAaAaAaAa", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "AaAaAaAa", exp: 0xAAAAAAAA, ok: true},
		{name: "", str: "0b01010101010101010101010101010101", exp: 0x55555555, ok: true},
		{name: "", str: "012525252525", exp: 0x55555555, ok: true},
		{name: "", str: "0o12525252525", exp: 0x55555555, ok: true},
		{name: "", str: "1431655765", exp: 0x55555555, ok: true},
